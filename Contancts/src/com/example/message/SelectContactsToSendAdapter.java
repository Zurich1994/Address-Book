...
public class SelectContactsToSendAdapter extends BaseAdapter {
	...
	public SelectContactsToSendAdapter(Context context, List<ContactInfo> list, QuickAlphabeticBar alpha) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		holder.qcb.assignContactUri(Contacts.getLookupUri(cb.getId(), cb.getLookUpKey()));
		if(0 == cb.getPhotoId()){
			holder.qcb.setImageResource(R.drawable.touxiang);
		}else{
			Uri uri = ContentUris.withAppendedId(ContactsContract.Contacts.CONTENT_URI, cb.getId());
